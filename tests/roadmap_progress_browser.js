...
async (page) => {
  ...
  if (await route.getByRole("region", { name: "节点 1", exact: true }).getByRole("checkbox").isEnabled()) {
  ...
  await page.reload();
  await route.getByText("学习进度：已完成 3 / 3， 剩余 0 个节点", { exact: true }).waitFor();
  const routes = await (await page.request.get("http://127.0.0.1:8102/api/roadmaps")).json();
  const current = await (await page.request.get(`http://127.0.0.1:8102/api/roadmaps/${routes[0].id}`)).json();
  const todos = await (await page.request.get("http://127.0.0.1:8102/api/todos")).json();
  ...
  const run = await (await page.request.get(`http://127.0.0.1:8102/api/runs/${current.nodes[2].completion.run_id}`)).json();
  const retry = await (await page.request.post(`http://127.0.0.1:8102/api/runs/${run.id}/retry`)).json();
  ...
  await route.screenshot({ path: "output/playwright/issue22-progress.png" });
