...
type ProposedNode = Pick<Node, "goal" | "estimated_minutes" | "source_ids" | "exercise" | "completion_criteria" | "todo_title" | "scheduled_date"> & { node_id: string | null };
type ChangeNode = Omit<ProposedNode, "node_id"> & { id: string; position: number };
...
function NodeChange({ label, node }: { label: string; node: ChangeNode | null }) {
  return node ? <div><strong>{label}：第 {node.position} 步 · {node.goal}</strong>
    <p>预计 {node.estimated_minutes} 分钟 · 日期：{node.scheduled_date || "未安排"}</p>
...
function Proposal({ proposal: p, route, disabled, act }: {
  ...
  return <article aria-label="调整方案" className="roadmap-confirmation">
    <h4>{p.status === "pending" ? "待确认调整" : p.status === "applied" ? "已应用调整（历史方案）" : "调整方案冲突，请重新生成"}</h4>
    ...
    {p.entries.map((e, i) => <section key={i}>
      ...
      {e.todo_before && <div>
        ...
        {p.status === "pending" && p.sync_todo_ids.includes(e.todo_before.id) && <label>
    ...
    {p.status === "pending" && <div className="roadmap-actions">
...
export function RevisionPanel({ route, disabled, act }: {
  ...
  const [nodes, setNodes] = useState<ProposedNode[]>(() => route.nodes.map(n => ({
    ...
    exercise: n.exercise, completion_criteria: n.completion_criteria, todo_title: n.todo_title, scheduled_date: n.scheduled_date,
  })));
  ...
  return <section aria-label="路线调整" className="roadmap-confirmation">
    <h4>调整学习路线</h4><p>先生成并审阅差异，确认后应用。拒绝同步时整份方案保持待确认。</p>
    ...
    <details><summary>手动编辑调整草稿（无需模型）</summary>
      ...
      {nodes.map((node, index) => {
        ...
        return <fieldset key={node.node_id || `new-${index}`}><legend>草稿节点 {index + 1}{completed ? "（已完成，内容受保护）" : ""}</legend>
          <fieldset disabled={disabled || completed}>
            ...
            <label>节点日期<input type="date" value={node.scheduled_date || ""} onChange={e => patch(index, { scheduled_date: e.target.value || null })} /></label>
      ...
      <button disabled={disabled || nodes.length >= 8} onClick={() => setNodes(previous => [...previous, {
        node_id: null, goal: "", estimated_minutes: 30, source_ids: route.sources.slice(0, 1).map(s => s.id), exercise: "", completion_criteria: "", todo_title: "", scheduled_date: null,
      }])}>新增候选节点</button>
