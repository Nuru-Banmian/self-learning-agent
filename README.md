...
已实现 Issue #2、#3：聊天创建、查询、修改和完成待办，按实际待办生成基础当天计划，明确接受后将行动建议加入待办；包含 SQLite 持久化、工作台及 HTTP/SSE。记忆、搜索和天气属于后续切片。首版面向本机单人使用。
...
- “新会话”保留待办，当前会话的行动建议独立展示。无模型配置时仍可查看列表，通过面板修改标题、日期、完成待办或接受已有建议；这些操作与聊天复用同一校验、事务及请求防重复规则。清空面板日期表示未安排。尚不支持自动记忆或执行任务。
...
测试通过公开 HTTP/SSE 使用真实 SQLite，只模拟外部模型或时钟，存储失败通过 SQLite 触发器注入。重启测试实际启动、终止并重新启动 Uvicorn 进程，且重启后移除模型凭据验证读取及建议防重复。验收证据与未验证项见 [Issue #2](docs/acceptance/issue-2.md) 和 [Issue #3 验收记录](docs/acceptance/issue-3.md)。
