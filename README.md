...
已实现 Issue #2、#3、#4：聊天创建、查询、修改和完成待办，按实际待办生成基础当天计划，明确接受后将行动建议加入待办；包含 SQLite 持久化、工作台及 HTTP/SSE。普通聊天可形成有来源的用户记忆，在新会话普通问答中使用。搜索、天气及记忆编辑删除属于后续切片。首版面向本机单人使用。
...
- 记忆面板展示内容、类别、范围、有效时间、生效情况和原始消息。相近主题的潜在冲突会暂停双方生效并提示澄清；明确纠正替换、编辑删除由后续 Issue #5 实现。
- 普通问答仅加载相关、生效的记忆：按主题词及中文相邻双字匹配，最多 6 条，序列化上下文最多 4000 字符，含来源消息和会话 ID。不会重放完整旧聊天；“这次”“本次”例外不加载一般偏好，也不永久改写偏好。关键词检索可能漏掉同义表达。
...
面板操作仍提交到 messages 接口，例：`action: {"tool":"update_todo","arguments":{"todo_id":"完整ID","title":"新标题","date_text":"2026-09-25"}}`。修改只提交需要变动的字段；`date_text: null` 清空日期。完成使用 `complete_todo` 和 `todo_id`；接受建议使用 `accept_suggestion` 和 `suggestion_id`。`content` 保存可读的操作描述。请求 ID 同时绑定文本、会话和 action，不能复用 ID 偷换操作。模型返回同类候选操作时，还必须通过原始聊天指令的授权与目标校验。
...
测试通过公开 HTTP/SSE 使用真实 SQLite，只模拟外部模型或时钟，存储失败通过 SQLite 触发器注入。重启测试实际启动、终止并重新启动 Uvicorn 进程，且重启后移除模型凭据验证读取及建议防重复。验收证据与未验证项见 [Issue #2](docs/acceptance/issue-2.md) 、[Issue #3](docs/acceptance/issue-3.md) 和 [Issue #4 验收记录](docs/acceptance/issue-4.md)。
