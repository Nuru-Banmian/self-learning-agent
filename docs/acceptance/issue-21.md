...
- 聊天支持“把路线 完整标识 全部加入待办”，唯一完整标题也可解析；歧义或未解析的路线/节点加入指令提示从面板选择。部分选择以面板为主。普通建议仍限当前会话。
...
- mypy、Ruff check/format、前端 TypeScript/Vite build、`git diff --check` 已通过。全量与双轴最终结果在完成后回填。
